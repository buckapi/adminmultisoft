...
export class AuthRESTService {
  ...
  constructor(private http: HttpClient,public yeoman:Yeoman) { }
	...
	registerUser( email: string, password: string){
		const url_api ='https://db.buckapi.us:9023/api/Users';
		return this.http
	...
	loginUser(email:string, password:string):Observable<any>{
		const url_api ='https://db.buckapi.us:9023/api/Users/login?include=user';
		return this.http
		.post<UserInterface>(url_api,{email,password},{headers:this.headers})
		.pipe(map(data => data));
	}
    ...
    let user_string = localStorage.getItem("currentUser");
	 ...
	 logoutUser(){
	  	let accessToken = localStorage.getItem('accessToken');
		  	const url_api = 'https://db.buckapi.us:9023/api/users/logout?access_token=${accessToken}';
